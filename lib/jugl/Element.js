 ...
 *
 * Parameters:
 * template - {jugl.Template} The template that this element belongs to.
 * node - {DOMElement} The DOM node that this element represents.
 *
 * Return:
 * {_jugl.Element} A new jugl element.
 */
_jugl.Element = function(template, node) {
...
_jugl.object.extend(_jugl.Element.prototype, {
    
    /**
     * Property: template
     * {<jugl.Template>} The template that this element belongs to.
     */
    template: null,
     ...
     *
     * Return:
     * {_jugl.Element} A cloned copy of this element.
     */
    clone: function() {
        ...
        if(this.node.nodeType === 1) {
            if(this.template.usingNS) {
                node = this.node.getAttributeNodeNS(
                    jugl.namespaceURI, localName
                );
            } else {
                node = this.node.getAttributeNode(
                    jugl.prefix + ":" + localName
                );
            }
     ...
     *
     * Return:
     * {Array(_jugl.Element)} An array of children elements.
     */
     ...
     *
     * Parameters:
     * element - {_jugl.Element} The node to import.
     */
