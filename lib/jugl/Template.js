...
_jugl.object.extend(jugl.Template.prototype, {
    ...
    /**
     * Property: regExes
     * {Object} Contains compiled regular expressions for use by the template.
     */
    regExes: {
        trimSpace: (/^\s*(\w+)\s+(.*?)\s*$/)
    },
    ...
    process: function(config) {
        ...
        config = _jugl.object.defaults(config, {
            context: null,
            clone: false,
            string: false
        });
        ...
        var element = new _jugl.Element(this, this.node);
        if(config.clone) {
        ...
        if(config.string) {
            ...
            if(config.parent) {
                if(config.clone) {
                    data = _jugl.node.appendChild(config.parent, element.node);
                } else {
    ...
    load: function(config) {
        ...
        _jugl.request.loadTemplate(config.url, setNode, this);
    },
    ...
    appendTo: function(parent) {
        this.node = _jugl.node.appendChild(parent, this.node);
        return this;
    }
