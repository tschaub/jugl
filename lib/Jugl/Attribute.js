    ...
    Jugl.Attribute = Jugl.Class({
        ...
        processAttribute: {
            ...
            "content": function() {
                ...
                if(pair[0] == 'structure') {
                    try {
                        this.node.element.innerHTML = str;
                    } catch(err) {
                        ...
                        if(this.node.element.xml && this.template.xmldom) {
                            while(this.node.element.firstChild) {
                                this.node.element.removeChild(
                            ...
                            try {
                                for(var i=0; i<children.length; ++i) {
                                    this.node.element.appendChild(children[i]);
                                }
                            } catch(invalidXML) {
                            ...
                            try {
                                this.node.element.innerHTML = wrapper.innerHTML;
                            } catch(invalidXML) {
                                msg = "Can't transform string into valid XHTML : " +
                                      str;
                                Jugl.Console.error(msg);
                    ...
                    var child = new Jugl.Node(this.template, text);
            ...
            "replace": function() {
                ...
                if(pair[0] == 'structure') {
                    ...
                    var child;
                    for(var i=0; i<children.length; ++i) {
                        this.node.element.parentNode.insertBefore(
                            children[i], this.node.element
                        );
                    }
                } else {
                    var text;
                    ...
                    var replacement = new Jugl.Node(this.template, text);
            ...
            "reflow": function() {
                ...
                this.removeSelf();
                if(reflow) {
                    if(this.node.element.outerHTML) {
                        this.node.element.outerHTML = this.node.element.outerHTML;
                    } else {
                        this.node.element.innerHTML = this.node.element.innerHTML;
                    }
                }
            }
